...
import { setProtaRegion } from '../../../redux/actions/protaActions';
    ...
    const setProta = (e) =>{
        ...
        return <div className= {`transform  hover:scale-110 duration-300 
                ...
                ${(prota.generation === generation)&&"opacity-100"}`}
                onClick={setProta} >
            <h1 className='font-Poppins text-lg sm:text-base md:text-xl lg:text-2xl font-bold text-center colorPokemon' >
                {names}
            </h1>

            <img alt={generation} src={sprites}
                className='w-44 md:w-36 lg:w-44 h-36 md:h-28 lg:h-36 rounded-xl mt-3 ' />
        </div>;
};
